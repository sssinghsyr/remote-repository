      ...
      while (true){
        ...
        if (msg.to().port != msg.from().port)  // avoid infinite message loop
		{
		  ...
		  if (reply.containsKey("file")){
			  Msg tempReply;
			  tempReply.to(msg.from());
			  tempReply.from(msg.to());
			  tempReply.command("fileSent");
			  tempReply.attribute("fileName", reply.value("file"));
			  postMessage(tempReply);
		  }
          msg.show();
          reply.show();
        }
